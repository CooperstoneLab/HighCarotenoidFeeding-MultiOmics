---
title: "Semipolar blood plasma lipidomics - Data filtering"
...
```{r}
metabdata <- read_csv(file = "features_postMZmine_lipidomicsNEG.csv",
...
MZ_RT <- MZ_RT %>%
  dplyr::select(-`row m/z`, -`row retention time`) 
```
...
MZ_RT_QCs <- MZ_RT %>%
  dplyr::select(`row ID`, mz_rt, contains("QC")) %>% # select QCs
...
MZ_RT_filt_PBremoved <- MZ_RT_filt_PB %>%
  ...
  dplyr::select(-fold_higher_in_QC, -avg_height_QC, -avg_height_PB, -(all_of(PBs)))
...
write_csv(MZ_RT_filt_PBremoved,
          "features_post-data-filtering.csv")
```
